...
function forceRefresh(){var tempUserName;var elements=document.getElementsByClassName("active");if(elements.length!=1){alert("No user has been selected.");return}else{tempUserName=elements[0].innerHTML}
...
user=username;var temp=document.getElementsByClassName("active");for(let i=0;i<temp.length;i++){temp[i].classList.remove("active")}
event.srcElement.classList.add("active");requestTask(username);if(document.getElementById("autoRefreshCheckBox").checked==!0){autoRefresh();for(let i=0;i<currentInterval.length;i++){clearInterval(currentInterval[i])}
...
function loadGoals(){try{if(xhttpRequest.readyState===XMLHttpRequest.DONE){if(xhttpRequest.status===200){var response=xhttpRequest.responseText;response=JSON.parse(response);loadGoalsTable(response)}else{alert('There was a problem with the request.')}}
...
return 1}catch(e){alert('Caught Exception: loadMemoText'+e.description)}}
...
function showGoals(){var x=document.getElementById("mainContainer4");var y=document.getElementById("goalsCheckBox");if(y.checked==!0){x.style.display="block"}else{x.style.display="none"}}
