...
if (!empty($_POST))
{
  ...
  foreach($infoArray as $arrayObject)
  {
    ...
    {
      $stmt = $conn->prepare("UPDATE crosscheck SET data1 = ?, data2 = 0 WHERE username = ? AND recordnum = ?");
    ...
    {
      $stmt = $conn->prepare("INSERT INTO crosscheck (recordnum, username, data1, data2) VALUES (?, ?, ?, 0)");
